<?php
/**
 * Formie Paragraph Field translation file (German)
 *
 * @link      https://lindemannrock.com
 * @copyright Copyright (c) 2025 LindemannRock
 */

return [
    // Plugin meta
    'Formie Paragraph Field' => 'Formie Paragraph Field',
    'Plugin Name' => 'Plugin-Name',
    'The name of the plugin as it appears in the Control Panel menu' => 'Der Name des Plugins, wie er im Control-Panel-Menü erscheint',

    // Field schema (form builder)
    'Choose the size for the paragraph text.' => 'Wählen Sie die Größe für den Absatztext.',
    'Paragraph Content' => 'Absatzinhalt',
    'Text Size' => 'Textgröße',
    'The content to be displayed. Line breaks will be converted to <br> tags.' => 'Der anzuzeigende Inhalt. Zeilenumbrüche werden in <br>-Tags umgewandelt.',

    // Common
    'Base' => 'Standard',
    'Extra Large' => 'Extra Groß',
    'Extra Small' => 'Extra Klein',
    'Large' => 'Groß',
    'Paragraph Field' => 'Absatz-Feld',
    'Paragraph content will appear here' => 'Absatzinhalt wird hier angezeigt',
    'Size:' => 'Größe:',
    'Small' => 'Klein',

    // Validation messages
    'Default text size must be one of: {values}' => 'Die Standard-Textgröße muss einer der folgenden Werte sein: {values}',

    // Settings: General
    'General Settings' => 'Allgemeine Einstellungen',
    'This is being overridden by the `pluginName` setting in the `config/formie-paragraph-field.php` file.' => 'Dies wird durch die Einstellung `pluginName` in der Datei `config/formie-paragraph-field.php` überschrieben.',

    // Settings: Default Field Settings
    'Custom Text Sizes' => 'Benutzerdefinierte Textgrößen',
    'Default Field Settings' => 'Standard-Feldeinstellungen',
    'Default Text Size' => 'Standard-Textgröße',
    'The default text size for new paragraph fields' => 'Die Standard-Textgröße für neue Absatz-Felder',
    'These settings will be used as defaults when creating new paragraph fields.' => 'Diese Einstellungen werden als Standardwerte beim Erstellen neuer Absatz-Felder verwendet.',
    'This is being overridden by the `defaultTextSize` setting in the `config/formie-paragraph-field.php` file.' => 'Dies wird durch die Einstellung `defaultTextSize` in der Datei `config/formie-paragraph-field.php` überschrieben.',
];
