# Quickstart

Add a block of static text to a Formie form. By the end you'll have content rendering between the form's input fields for visitors to read.

## Before you start

Complete [Installation & Setup](installation.md#post-install-setup) first. You should have:

- Formie Paragraph Field and [Formie](https://verbb.io/craft-plugins/formie) installed and enabled
- A Formie form you can edit

## 1. Add a Paragraph field

In the Control Panel, open **Formie → Forms** and edit (or create) a form. Drag **Paragraph** from the field list onto a page.

## 2. Write the content

In the field's settings:

1. Give it a **Name** (form-builder label only — not shown to visitors).
2. Type your text into **Paragraph Content**. Line breaks become `<br>`.
3. Pick a **Text Size**.

Save the form.

## 3. Verify it works

Open the form on your site front-end (or use Formie's preview). Your paragraph appears as static text between the other fields — there's nothing to fill in; it's display-only content.

## What's next

- [The Paragraph field](../feature-tour/paragraph-field.md) — every option and how it renders
- [Configuration](configuration.md) — set a default size and add your own text sizes
- [Feature tour](../feature-tour/overview.md) — what the plugin does
